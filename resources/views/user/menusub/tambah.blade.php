...
<main>
    <div class="container-fluid">
        <h1 class="mt-4">Tambah Sub Menu Navigasi</h1>
        <ol class="breadcrumb mb-4">
            <li class="breadcrumb-item active">Sub Menu - Tambah</li>
        </ol>
        <!-- row two -->
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form action="{{url('admin/menusub/simpan')}}" method="POST">
                    @method('PUT')
                    @csrf
                      <div class="mb-3">
                          ...
                          <select name="menu_id" class="form-select form-select-sm @error('menu_id') is-invalid @enderror" aria-label=".form-select-sm">
                              ...
                              @foreach ($menu as $men)
                                  <option value="{{ $men->id}}" {{ old('menu_id') == $men->id ? 'selected' : null }}>{{ $loop->iteration }}. {{$men->judul}} </option>
                              @endforeach
                          </select>
                          @error('menu_id')
                      ...
                      <div class="mb-3">
                          <a href="{{url('admin/menusub')}}" class="btn btn-danger">Kembali</a>
                      </div>
                  </form>
            </div>
        </div>
